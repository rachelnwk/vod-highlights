...
$archivePath = Join-Path $PSScriptRoot $ZipFile
New-BackendArchive -ArchivePath $archivePath

Write-Host ""
Write-Host "3. Creating environment on EB..."
$createArgs = @(
    $EnvName,
...
if ($ServiceRole) {
    $createArgs += @("--service-role", $ServiceRole)
}

if ($VpcId -and $VpcSubgroups) {
    $createArgs += @("--vpc.id", $VpcId, "--vpc.ec2subnets", $VpcSubgroups)
}

& eb create @createArgs

Write-Host ""
Write-Host "4. Deploying app to EB..."
