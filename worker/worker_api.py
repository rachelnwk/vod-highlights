...
# Build the local working-directory path for one job.
...
# Convert one stored clip record into the frontend API response shape.
...
# Convert one stored video/job record into the frontend API response shape.
...
# Update one in-memory job, persist it to MySQL, and return a snapshot.
...
# Read one in-memory job snapshot if it still exists.
...
# Remove a completed or failed job from the in-memory active job map.
...
# Count how many in-memory jobs are currently queued or processing.
# Input: No arguments.
...
# Delete intermediate frame and crop directories for one job.
...
# Save a JSON artifact for debugging the analysis request/response.
...
# Run the full local processing pipeline for one uploaded video job.
...
# Add permissive CORS headers to every worker API response.
...
# Report worker health, config readiness, and active job count.
...
# Accept a file upload, create a queued job, and start background processing.
...
# List saved videos or delete all saved videos and clips.
...
# Delete one saved clip from S3 and the database.
...
# Download selected clips from S3, merge them locally, and send one output file.
...
def merge_library_clips():
    ...
    try:
        ...
        # Delete the temporary merge directory after the response is sent.
